...
class ILVRC2012_Set:
	def __init__(self, paramfile):
		print "ilvrc2012_imagebbx_mv with paramfile:", paramfile
		self.param = {'paramfile': paramfile, 'imgsize': 224, 'class': -1, 'test': 0, 'LR': 0}
	...
	def get_input_dim(self, idx):
            ...
            elif idx==1: #spatial infor (x, y, s)
                return 3
            else:
                assert(0)
                return 0
	...
	def get_inputs(self, idx):
		...
		if self.curimgidx==imgidx:
			...
			if len(img.shape) == 2:
				...
				if img.shape[2] == 4:
					img = img[:,:,:3]
                        if self.param['LR']>0: #convert to low-res scale
                            h, w, c = img.shape
                            lrs=self.param['LR']
                            ratio = 1.0*lrs / min(w,h)
                            img = scipy.misc.imresize(img, (max(lrs, int(h*ratio)), max(lrs, int(w*ratio))))
		...
		if crop=='wh':
			...
			cidx = crop[1]%crop[0]
			ridx = crop[1]/crop[0]
                        xoff = 1.0*cidx/crop[0]
                        yoff = 1.0*ridx/crop[0]
			l = int(scale*min(w, h))
		...
		return [img-self.meanImg, np.array([xoff, yoff, scale])]
...
def test(param):
	...
	for i in range(0, 16, 1):
		im = ts.get_inputs(i)
		bbx = ts.get_output(i)
		print i, bbx[0], im[0].shape, im[1]
		b += [bbx[0]]
		scipy.misc.imsave('./img/{}.png'.format(i), im[0])
	sio.savemat('./img/bbx.mat', {'bbx': b})

if __name__ == '__main__':
	print 'testing ilvrc2012_imagebbx_mv.py!'
